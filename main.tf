resource "aws_cloudwatch_metric_alarm" "alb" {
  ...
  dimensions = contains(keys(element(var.alb_alarms, count.index)), "dimensions") ? merge(lookup(element(var.alb_alarms, count.index), "dimensions"), {LoadBalancer = data.aws_lb.alb.arn}) : {LoadBalancer = data.aws_lb.alb.arn}
}